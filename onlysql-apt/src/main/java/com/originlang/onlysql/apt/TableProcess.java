...
class TableProcess {
    ...
    private List<FieldSpec> getFields(TypeElement entityClass) {
        ...
        // table name
        FieldSpec tableName = FieldSpec.builder(
//                                TypeName.get(element.asType()),
                        String.class,
                      "tableName",//  entityClass.getSimpleName().toString(),
                        // element.getModifiers().toArray(new Modifier[0])
                        Modifier.PUBLIC, Modifier.STATIC, Modifier.FINAL)
                .initializer("$S", entityClass.getSimpleName().toString())
                .build();
        fieldSpecs.add(tableName);
